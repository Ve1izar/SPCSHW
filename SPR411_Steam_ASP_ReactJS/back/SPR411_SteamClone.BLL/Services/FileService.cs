...
namespace SPR411_SteamClone.BLL.Services
{
    public class FileService
    {
        ...
        public void DeleteFile(string fileName, string destPath)
        {
            if (string.IsNullOrEmpty(fileName)) return;
            
            string filePath = Path.Combine(_storagePath, destPath, fileName);
        ...
        public void DeleteFolder(string destPath)
        {
            if (string.IsNullOrEmpty(destPath)) return;

            string fullPath = Path.Combine(_storagePath, destPath);
            if (Directory.Exists(fullPath))
            {
                Directory.Delete(fullPath, true); // true - видалити папку разом з усім вмістом
            }
        }
    }
}
