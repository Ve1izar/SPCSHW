...
namespace SPR411_SteamClone.BLL.Services
{
    public class DeveloperService
    {
        ...
        public async Task<ServiceResponse> UpdateAsync(UpdateDeveloperDto dto)
        {
            ...
            if (dto.Image != null)
            {
                // видалення старого, навявного, зображення
                if (!string.IsNullOrEmpty(entity.Image))
                {
                    _fileService.DeleteFile(entity.Image, StaticFilesSettings.Developers);
                }
        ...
        public async Task<ServiceResponse> DeleteAsync(int id)
        {
            ...
            if (!res) return ServiceResponse.Error($"Не вдалося видалити розробника '{entity.Name}'");
